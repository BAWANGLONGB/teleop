# Marvin 14-DoF unified URDF

`marvin_dual.urdf` is the unified model used by the Marvin dual-arm teleoperation
stack. It contains one fixed common base, seven revolute joints per arm, and
explicit `left_tcp` and `right_tcp` frames.
...
exact gripper mesh checksums, rest-target FK alignment, and model loading in
MuJoCo and Placo/Pinocchio.

Validated in this workspace with MuJoCo 3.12.0 and Placo/Pinocchio 2.7.0. The
runtime MJCF adds two raw-target mocap bodies, two limited-command mocap bodies,
and two flange payload bodies while preserving the 14-DoF contract:

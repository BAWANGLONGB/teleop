...
12. 启用 `--nsp-lateral` 后，启动时缓存两臂参考平面；Grip 按下瞬间建立横向零点，
    手柄横向位移映射为最大 `5°` 的 `ZSP_Angle`，普通 IK 后调用 IK_NSP。角度按
    斜率渐变，NSP 失败或单步变化过大时回退普通 IK；旧的
    `--nsp-angle-left/right` 固定角度参数仍应保持兼容。

# Project Memory


## Decision Record: ROS topic 契约单点归属

**日期**: 2026-09-14
**问题**: ROS topic 字符串分散在发布、订阅、校验、后处理和审阅模块，修改容易遗漏。

### 选项分析

| 选项 | 优势 | 劣势 | 复杂度 |
| --- | --- | --- | --- |
| 复用 `ros/protocol.py` | 已是消息契约入口，一次检索可定位 | 配置 JSON 仍需显式保存字符串 | 低 |
| 新建 constants/geometry 模块 | 分类更细 | 增加模块和跳转，不解决更多问题 | 中 |

### 决策

**选择**: 复用 `ros/protocol.py`。
**理由**: 这是现有代码中的规范实现位置，改动最小且调用方明确。
**Trade-offs**: `protocol.py` 顶部常量增加；用户配置 JSON 保持可读、可快照，不强行代码生成。

### 影响范围

- `xr-marvin-teleop/xr_marvin_teleop/ros/protocol.py` 及 ROS、Episode、采集调用方。
- `xr-marvin-teleop/docs/project-structure.md` 和接口规范。

### 撤销条件

当协议按版本拆包，或出现独立于 ROS 的第三种传输协议时，再拆出共享数据契约模块。

## Decision Record: AV1 最终视频存储

**日期**: 2026-09-15
**问题**: 原生 MJPEG 最终文件体积较大，同时不能让视频压缩影响实时采集。

### 选项分析

| 选项 | 优势 | 劣势 | 复杂度 |
| --- | --- | --- | --- |
| 在线 AV1 编码 | 不保留大体积 MJPEG 分片 | 编码负载进入实时链路，失败时恢复困难 | 高 |
| 离线 AV1 编码 | 采集链路不变，保留源数据并可重试 | 导出需要额外时间和临时磁盘 | 中 |
| 继续默认 MJPEG | 无新增编码成本 | 最终文件体积不降 | 低 |

### 决策

**选择**: 在线保持 V4L2 原生 MJPEG，停止录制后使用 `libsvtav1` 离线生成 AV1 MCAP。
**理由**: 将高 CPU 编码移出实时路径，并通过原始 bag 和临时目录保障失败可恢复。
**Trade-offs**: 导出有编码延迟；SVT-AV1 的缓存帧先按 PTS 写入临时 SQLite，再按原消息顺序生成 MCAP。

### 影响范围

- `xr-marvin-teleop/xr_marvin_teleop/common/episode_video.py`、采集配置、后处理、迁移及收集脚本。
- `UI` 导出接口和格式选择、视频集成测试及操作文档。

### 撤销条件

真实双路相机基准表明离线耗时或临时磁盘不可接受，或部署环境无法稳定提供 `libsvtav1` 时，恢复 H.264 默认输出并保留 AV1 可选项。

## Decision Record: Marvin 关节阻抗模式确认

**日期**: 2026-09-18
**问题**: Marvin 同步 ACK 可能超时，但模式切换命令已被控制器执行。

### 选项分析

| 选项 | 优势 | 劣势 | 复杂度 |
| --- | --- | --- | --- |
| 同步 ACK 判定 | 立即获知 SDK 回显 | ACK 通道异常会误报模式失败 | 低 |
| 异步发送后检查实时反馈 | 与厂商 Demo 一致，验证实际控制状态 | 需要等待反馈稳定 | 低 |

### 决策

**选择**: 参数、模式切换、PD 前馈配置和启动位置保持均使用异步 `send_cmd()`；按厂商流程等待并以实时反馈确认状态。
**理由**: 实时反馈直接反映机器人是否进入关节阻抗模式，避免把 ACK 回显故障误判为模式失败。
**Trade-offs**: 参数配置后等待 0.5 秒，模式切换与 PD 前馈配置后各等待 1 秒；不再通过同步 ACK 立即确认这些启动命令。

### 影响范围

- `src/xr_marvin_teleop/adapters/marvin.py`
- `src/xr_marvin_teleop/control/controller.py`
- `src/xr_marvin_teleop/cli/reset.py`

### 撤销条件

厂商修复并验证同步 ACK 通道可靠，且明确要求模式切换必须依赖 ACK 时，恢复同步等待。

## Decision Record: 自动 H.264 MCAP 按日发布

**日期**: 2026-09-18
**问题**: 录制结束后需自动转换 H.264 MCAP，并放入 `var/collection/YYYY-MM-DD/`。

### 选项分析

| 选项 | 优势 | 劣势 | 复杂度 |
| --- | --- | --- | --- |
| 复制转换结果 | 目标文件完全独立 | 大 MCAP 双份占用空间 | 低 |
| 硬链接发布 | 原子、无额外数据占用，删除原 episode 后目标仍保留 | 要求源和目标在同一文件系统 | 低 |

### 决策

**选择**: 转换器继续写 episode `final/`，完成后硬链接到录制开始日期目录。
**理由**: 复用现有经验证的 postprocess 和导出锁，同时避免视频数据重复占盘。
**Trade-offs**: `DATASET_ROOT` 与 `var/collection` 必须位于同一文件系统；跨文件系统时自动导出会标记失败并保留原始产物。

### 影响范围

- `src/xr_marvin_teleop/web/server.py`（`teleop-web` 生产入口）
- `ui/server.py`（直接运行入口）
- `tests/test_ui_export.py`

### 撤销条件

当数据集根目录迁移到其他文件系统，改为带临时文件和原子重命名的流式复制。

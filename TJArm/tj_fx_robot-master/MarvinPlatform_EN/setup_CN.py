...
ui_entry = os.path.join(base_dir, 'ui_CN.py')
...
def move_executable():
    """将中文版可执行文件从 dist 拷贝到 MarvinPlatform_EN 目录下。"""
...
# 可执行文件名 / 图标
if sys.platform == 'win32':
    default_exe_name = 'MarvinPlatform_win_100343_CN'
    icon_ext = 'ico'
    bin_exts = ('.dll',)
else:
    default_exe_name = 'MarvinPlatform_linux_100343_CN'
    icon_ext = 'png'
    bin_exts = ('.so',)

# Allow a new build to coexist with the vendor-provided executable.
exe_name = os.environ.get('MARVIN_APP_NAME', default_exe_name)
...
# 内置简体中文字体，确保急停图片以及无完整字体回退的机器上不显示方框。
cjk_font_candidates = [
    '/usr/share/fonts/opentype/noto/NotoSansCJK-Bold.ttc',
    '/usr/share/fonts/truetype/droid/DroidSansFallbackFull.ttf',
]
cjk_font_src = next((path for path in cjk_font_candidates if os.path.isfile(path)), None)
if cjk_font_src is None:
    raise FileNotFoundError('打包中文版需要 Noto Sans CJK 或 Droid Sans Fallback 字体')
pack_args.extend(['--add-data', cjk_font_src + path_sep + 'fonts'])
print(f"  add-data: {cjk_font_src}")

# ============================================================
# 执行
# ============================================================

print(f"\nBuilding Chinese Marvin Platform for platform: {sys.platform} ...")
PyInstaller.__main__.run(pack_args)

move_executable()
clean_build_artifacts()

print("\n=== Chinese packaging completed! Executable is in MarvinPlatform_EN/ ===")
